var stop, staticx;
var img = new Image();
var skrcanvas,SHeight,SWidth;
const addb = 40;// 边缘超出的大小
...
function getRandom(option) {
    ...
    switch (option) {
        case 'x':
            ret = Math.random() * SWidth;
            break;
        case 'y':
            ret = Math.random() * SHeight;
            break;
        case 's':
            ret = Math.random();
            break;
...
function startSakura() {
    ...
    var cxt = skrcanvas.getContext('2d');
    ...
    stop = requestAnimationFrame(function () {
        cxt.clearRect(0, 0, skrcanvas.width, skrcanvas.height);
        sakuraList.update();
        sakuraList.draw(cxt);
